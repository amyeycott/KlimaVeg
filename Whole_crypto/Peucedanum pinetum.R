...
Picea1990 <- subset(vascOld.fat, select = "Picea_abies")
Picea2015 <- subset(vascNew.fat, select = "Picea_abies")
Summaries.ss.PP<-Reduce(HDRmerge, list(summaries.ss, Picea1990, Picea2015))# hdr merge is just a tidier version of merge, already composed in the source file. 

#set the colours for each year before making any plots. It'sthe klimaveg colours!
coloury<-cbind(c(1990,2015),c("#3873AE","#EF9335"))


library(dplyr)
Summaries.ss.PP<-rename(Summaries.ss.PP, Picea1990=Picea_abies.x , Picea2015=Picea_abies.y)
rm(Picea1990, Picea2015)#just to avoid confusion

table(Summaries.ss.PP$Picea1990,Summaries.ss.PP$Picea2015)#there were no Picea-free plots in either year, according to this! I checked in the original thin files

Summaries.ss.PP$Transition<-paste(Summaries.ss.PP$Picea1990, "to", Summaries.ss.PP$Picea2015)
...
mapply(function(x, main, ylim){boxplot(x~Transition, data=Summaries.ss.PP, main=main, cex.main=0.8, ylim=ylim)}, x = Summaries.ss.PP[,c("lich.BCdiss","lich.Sodiss","lich.rich1990","lich.rich2015","richchange","lich.extinct","lich.colonise","bryo.BCdiss","bryo.Sodiss","bryo.rich1990","bryo.rich2015","bryochange","bryo.extinct","bryo.colonise","vasc.BCdiss","vasc.Sodiss","vasc.rich1990","vasc.rich2015","vascchange","vasc.extinct","vasc.colonise")], main = c("Lichen BC diss", "Lichen Sörensen diss","Lichen plot richness 1990", "Lichen plot richness 2015","Lichen change in plot richness","Lichen plot extinctions since 1990", "Lichen plot colonisations 2015", "Bryophyte BC diss","Bryophyte Sörensen diss", "Bryophyte plot richness 1990", "Bryophyte plot richness 2015","Bryophyte change in plot richness","Bryophyte plot extinctions since 1990", "Bryophyte plot colonisations 2015", "vascular BC diss","vascular Sörensen diss", "vascular plot richness 1990", "vascular plot richness 2015","vascular change in plot richness","vascular plot extinctions since 1990", "vascular plot colonisations 2015"), ylim=list(c(0,0.35),c(0,0.55),c(0,120),c(0,120),c(-10,50),c(0,0.8),c(0,0.8)))#data exploration. 
...
mapply(function(x, z, main, ylim){
  ...
  x = Summaries.ss.PP[,c("bryo_threatened_1990","vasc_threatened_1990")], z=Summaries.ss.PP[,c("bryo_threatened_2015","vasc_threatened_2015")] , main=c("Threatened bryophytes","Threatened vascular"), ylim=list(c(0,50),c(0,10),c(0,2)))#note, ylim can't be moved back
...
mapply(function(x, z, main, ylim){
  boxplot(x~Transition, data=Summaries.ss.PP, main=main, ylim=ylim, at=c(1,3,5,7,9,11), xlim=c(0.5,12.5), col="#3873AE", cex.main=1)
    ...
    x = Summaries.ss.PP[,c("bryo.rich1990","vasc.rich1990")], z=Summaries.ss.PP[,c("bryo.rich2015","vasc.rich2015")] , main=c("Bryophyte richness","Vascular plant richness"), ylim=list(c(0,100),c(0,150)))
...
mapply(function(x, z, main, ylim, yaxp){  
  ...
  boxplot(z~Transition, data=Summaries.ss.PP, main=NULL,add=TRUE, at=c(1.8,3.8,5.8,7.8,9.8,11.8), col="#EF9335", xaxt="n", yaxt="n")}, 
  x = Summaries.ss.PP[,c("bryo_threatened_1990","vasc_threatened_1990")], z=Summaries.ss.PP[,c("bryo_threatened_2015","vasc_threatened_2015")] , main=c("Threatened bryophytes","Threatened vascular"), ylim=list(c(0,10),c(0,2)),yaxp=list(c(0,10,2),c(0,2,1)))
...
legend("topright", fill=c("#3873AE","#EF9335"), legend=c("1990","2015"), y.intersp=0.8)
...
#####ORDINATIONS#####
library(vegan)
giantdataset<-Reduce(HDRmerge, list(subset(comp, select=-Year), bryo.fat, vascall.df))
...
ords.ss.sites<-merge(lichnmds.ss.sites, Summaries.ss.PP[,c("Picea1990", "Picea2015","Transition")], by.x="plot", by.y=0)
...
ords.ss.sites$Picea1990<-factor(ords.ss.sites$Picea1990, levels = 3:1)
...
plotty+facet_grid(-Picea1990~-Picea2015)
...
#plot.lich<-ggplot(ords.ss.sites, aes(lichnmds1, lichnmds2, colour=year))+  geom_point()+facet_grid(-Picea1990~-Picea2015)+ coord_fixed()
...
plot.bryo<-ggplot(ords.ss.sites, aes(bryonmds1, bryonmds2, colour=year))+  
  ...
  facet_grid(Picea1990~Picea2015)+ 
  coord_fixed()+
