...
means.Fe<-c(mean(colSums(FeLS1990table.byplot)), mean(colSums(FeLS2015table.byplot)), mean(colSums(FeCWD2015table.byplot)))
sd.Fe<-c(sd(colSums(FeLS1990table.byplot)), sd(colSums(FeLS2015table.byplot)), sd(colSums(FeCWD2015table.byplot)))
plot(means.Fe, ylab="mean plot species richness", xaxt="n", ylim=c(0,30), xlab=NA)
axis(1, labels=c("LS1990","LS2015","CWD2015"), at=c(1,2,3))
...
axis(1, labels=c("LS1990","LS2015","CWD2015"), at=c(1.2,2,2.8), cex=1.5) 
...
dim(FeLS1990table.byplot) #66 x 64 sixty six species in 64 plots
dim(FeLS2015table.byplot)#66 x 52 sixty seven species in 52 plots, note 60 in common with FeLS1990
...
by.LS1990<-by(hosts.LS1990$hosts.LS1990, hosts.LS1990$Plot, mean)
...
errbar(c(1.2,2,2.8),c(mean(by.LS1990), mean(by.LS2015), mean(by.CWD2015)),c(mean(by.LS1990)+sd(by.LS1990),mean(by.LS2015)+sd(by.LS2015),mean(by.CWD2015)+sd(by.CWD2015)),c(mean(by.LS1990)-sd(by.LS1990),mean(by.LS2015)-sd(by.LS2015),mean(by.CWD2015)-sd(by.CWD2015)),xaxt="n", xlab=NA, xlim=c(1,3), main="Hosts per ash-epiphytic bryophyte species per plot", ylab="n hosts +- 1 sd", cex.axis=1.2, cex.lab=1.2, cex=1.2, col=c("forestgreen", "forestgreen", "chocolate4"), errbar.col=c("forestgreen", "forestgreen", "chocolate4"), lwd=2)
axis(1, labels=c("LS1990","LS2015","CWD2015"), at=c(1.2,2,2.8), cex=1.5) #this is Figure 2 in the summary document.
...
#In the paper format
x11(4,6); par(mar=c(3,4,2,0.1), mfrow=c(2,1))
errbar(c(1.2,2,2.8),means.Fe, (means.Fe+sd.Fe) , (means.Fe-sd.Fe), ylim=c(0,40), pch=16, ylab="mean plot species richness +- 1 sd", xlab=NA, xaxt="n", xlim=c(1,3))
title(main="a)", adj=0)
axis(1, labels=c("LS1990","LS2015","CWD2015"), at=c(1.2,2,2.8)) 
errbar(c(1.2,2,2.8),c(mean(by.LS1990), mean(by.LS2015), mean(by.CWD2015)),c(mean(by.LS1990)+sd(by.LS1990),mean(by.LS2015)+sd(by.LS2015),mean(by.CWD2015)+sd(by.CWD2015)),c(mean(by.LS1990)-sd(by.LS1990),mean(by.LS2015)-sd(by.LS2015),mean(by.CWD2015)-sd(by.CWD2015)),xaxt="n", xlab=NA, xlim=c(1,3), ylab="mean alternative hosts per plot +- 1 sd")
title(main="b)", adj=0)
axis(1, labels=c("LS1990","LS2015","CWD2015"), at=c(1.2,2,2.8)) 
...
sitescores.ash.dca[(sitescores.ash.dca$DCA1)>1,]#big outliers are C11990, E012015, G081990
...
points(ash.nmds, col=as.factor(substr(rownames(ashtabx),4,7)))# 1990 is black and 2015 is red. nice - this is Figure 3 in the summary document. 
...
anova(betatest)#no it's not, F1,113=0.295, p= 0.5883. But am I testing what I think I am? I think I'm testing whether the diversity between plots got more or less. That's not the right question.

#is 2015 CWD more like 1990 FE_LS or like 2015 Fe_LS? 
...
constantsites<-intersect(constantsites,colnames(FeLS1990table.byplot))
  
CWDnmds<-metaMDS(t(FeCWD2015table.byplot[,which(colnames(FeCWD2015table.byplot)%in%constantsites)]))
FeLS1990nmds<-metaMDS(t(FeLS1990table.byplot[,which(colnames(FeLS1990table.byplot)%in%constantsites)]))
...
protest(CWDnmds, FeLS1990nmds)# correlation 0.037, P=0.944!!!
...
FeLS1990dist<-vegdist(t(FeLS1990table.byplot[,which(colnames(FeLS1990table.byplot)%in%constantsites)]))#72 35
...
mantel(CWD2015dist, FeLS1990dist)# r=0.1205, P=0.195

allindist<-vegdist()

####Rank-ocurrence#####
occurrencesnew<-as.data.frame(rowSums(FeLS2015table.byplot))
occurrencesold<-as.data.frame(rowSums(FeLS1990table.byplot))#not perfectly overlapping, depsite containing the same number of species they are different
occurrences<-merge(occurrencesold, occurrencesnew, by=0, all=TRUE)
names(occurrences)<-c("Species","FeLS1990","FeLS2015")
head(occurrences)
...
barplot(sort(occurrences$FeLS1990, decreasing=TRUE), ylim=c(0,60), xlab="Species ranked by occurrences", ylab="Total occurrences", main="both years sorted")
barplot(sort(occurrences$FeLS2015, decreasing=TRUE), col="yellow", add=TRUE)
legend("topright", legend=c("1990", "2015"), fill=c("grey","yellow"))

#plot where each bar represents a specific species, regardless of year
occurrences.ordered<-occurrences[(order(occurrences$FeLS1990, decreasing=TRUE)),]#this is the line to be suspicious of.
barplot(occurrences.ordered$FeLS1990, ylim=c(0,60), xlab="Species ranked by occurrence in 1990", ylab="Total occurrences", main="each bar is one species")
...
#winners, those who increased from fewer than 5 plots in 1990 to more than 5 plots in 2015
occurrences[(occurrences$FeLS1990)<5&(occurrences$FeLS2015)>5,]
#losers, those who decreased from more than 5 plots in 1990 to fewer than 5 plots in 2015
occurrences[(occurrences$FeLS1990)>5&(occurrences$FeLS2015)<5,]
...
generalisty.1990<-sapply(hostsall.1990[, 2:(length(hostsall.1990)-1)], function(x){
  mean(hostsall.1990$max[x==1])
})
generalisty.1990
t.test(hostsall.2015$max[hostsall.2015$Fe_LS==1], hostsall.1990$max[hostsall.1990$Fe_LS==1])
...
#there are a bunch on the left in 1990 not in 2015 - are these the plots which lost their Fe???
...
#asked for: Histogram of total n alternative hosts and barplot of ash epiphytes per host. Also for CWD? (3x2 – LS 1990, LS2015, CWD 2015).
x11(6,3); par(mfrow=c(1,3), xpd=NA, cex=1.2, mgp=c(2,0.7,0), mar=c(4,3,3,1))
hist(hostsall.1990$max[hostsall.1990$Fe_LS==1], xlim=c(0,14), ylim=c(0,25), las=1, xlab="", main="1990", col="forestgreen")
...
barplot(colSums(hostsall.1990[,substr(colnames(hostsall.1990),4,5)=="LS"]), names.arg =substr(colnames(hostsall.1990[,substr(colnames(hostsall.1990),4,5)=="LS"]),1,2), las=2, xlab="", ylab="Number of epiphyte species", col="forestgreen", main="1990")
